  ...
  return (
    <>
      <section className="min-h-screen bg-[radial-gradient(hsl(0,72%,65%,40%),hsl(24,62%,73%,40%),hsl(var(--background))_60%)] flex items-center justify-center text-center text-balance flex-col gap-8 px-4">
        ...
        <p className="text-lg lg:text-3xl max-w-screen-xl">Optimize your product pricing accross countries to maximize sales. Capture 85% of the untapped market with location-based dynamic pricing.</p>
        <SignUpButton>
          <Button className="text-lg p-6 rounded-xl flex gap-2">Get Started for free <ArrowRightIcon className="size-5" /></Button>
        </SignUpButton>
      </section>
      <section className="bg-primary text-primary-foreground">
        <div className="container mx-auto py-16 flex flex-col gap-16 px-8 md:px-16">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-5 gap-16">
            ...
            <Link href="https://www.clerk.com" className="md:max-xl:hidden">
              <ClerkIcon />
            </Link>
          </div>
        </div>
...
function PricingCard({ name, priceInCents, maxNumberOfProducts, maxNumberOfVisits, canAccessAnalytics, canCustomizeBanner, canRemoveBranding }: (typeof subscriptionTiersInOrder)[number]) {
  ...
  return <Card>
    ...
    <CardFooter className="flex flex-col gap-4 items-start">
      <Feature className="font-bold">{maxNumberOfProducts === 1 ? "1 product" : `${maxNumberOfProducts} products`}</Feature>
